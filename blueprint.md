...
- **기능**:
    - **[v0.4.1]** 카테고리 페이지(`category.html`) 신설:
        - 특정 카테고리에 속한 퀴즈 목록을 페이지네이션과 함께 표시.
        - 해당 카테고리의 'HOT PIX'를 슬라이더 형태로 제공.
    - 실시간 투표 및 투표 상태 복원 기능.
    - **[v0.4.1]** `likes` 모듈을 통한 좋아요 기능의 모듈화 및 상태 관리 개선.
    - Firebase Email/Password 및 Google 소셜 로그인 기능.
    - `alert`를 제거하고 모달 내 전용 메시지 영역을 통해 일관된 피드백 제공.

- **데이터베이스 (Firestore)**:
    - `users` 컬렉션: 회원가입 시 사용자 문서(`users/{userId}`)를 생성하고 `displayName`과 초기 `points` (0)를 저장.
    - `firestore.rules`: `users` 컬렉션에 대한 보안 규칙을 추가하여 사용자가 자신의 포인트는 수정할 수 없도록 제한.
...
1.  **퀴즈 문서 스키마 확장 (`questions/{quizId}`)**
...
**마이그레이션 고려사항**: 기존 `questions` 문서에는 새로운 상태 필드들이 존재하지 않습니다. 애플리케이션 로직은 이 필드들이 `null` 또는 `undefined`일 경우를 항상 대비하여 작성되어야 합니다. 기존 `users` 컬렉션에서 `userProfiles`로의 데이터 마이그레이션은 추후 별도의 계획으로 진행될 것입니다.